...
const cfg = {
  ...
  numberOffset: 5,
  shadowBlur: true,
  bgColor: '#f0f0f0' // Changed to match your light theme
}
...
class SignalCanvas {
  constructor() {
    this.state = {
      ...
      planeProgress: 0,
      dotsProgress: 0,
      fadeInProgress: 1, // Start at 1 for instant visibility
      textProgress: 0,
      stepOffset: 0,
      textOffset: 0,
      markupOffset: 0,
      ...
      animNumbers: [],
      tabIsActive: true,
      planeIsDrawn: true, // Start as true
      mousePower: 0,
      delta: 0,
      dlt: performance.now(),
      needRedraw: true
  ...
  bindNodes() {
    ...
    if (!document.getElementById('signal-plane-canvas')) {
      const planeCanvas = document.createElement('canvas')
      planeCanvas.id = 'signal-plane-canvas'
      planeCanvas.style.position = 'fixed'
      planeCanvas.style.top = '0'
      planeCanvas.style.left = '0'
      planeCanvas.style.width = '100%'
      planeCanvas.style.height = '100%'
      planeCanvas.style.pointerEvents = 'none'
      planeCanvas.style.zIndex = '0'
      document.body.appendChild(planeCanvas)
    }

    if (!document.getElementById('signal-main-canvas')) {
      const mainCanvas = document.createElement('canvas')
      mainCanvas.id = 'signal-main-canvas'
    ...
    this.planeCanvas = document.getElementById('signal-plane-canvas')
    this.mainCanvas = document.getElementById('signal-main-canvas')
  }

  canvasInit() {
    const font = '10px Inter, Montserrat, sans-serif'
    const lineCapAndJoin = 'round'
    const color = `rgba(0,0,0,0.1)` // Black for light theme

    context.plane = this.planeCanvas.getContext('2d')
    ...
    context.main = this.mainCanvas.getContext('2d')
  ...
  resizeHandler() {
    ...
    this.planeCanvas.height = doc.height
    this.planeCanvas.width = doc.width
    this.updatePlane()
  ...
  loop() {
    const loop = () => {
      ...
      context.plane.clearRect(0, 0, doc.width, doc.height)
      this.updateState()
      this.draw()
  ...
  updateState() {
    ...
    if (mouse.down.state) {
      ...
      if (state.mousePower >= 1) state.mousePower = 1
    } else {
      state.mousePower -= 0.001 * dt
      if (state.mousePower <= 0) state.mousePower = 0
    }

    state.stepOffset += (0.002 * dt) + (state.mousePower * (0.0035 * dt))
    state.textOffset += (0.00005 * dt) + (state.mousePower * (0.002 * dt))
    state.markupOffset += (0.00015 * dt) + (state.mousePower * (0.00035 * dt))
  }
  ...
  startGeneratingNumbers() {
    ...
    const generateItem = () => {
      ...
      const timeToNewItem = tools.random(
        (1 + 50 * (1-mp)),
        (5 + 100 * (1-mp))
      ) / Math.max(state.area, 0.1)
      
      const item = {
        p: 0,
        color: `rgba(0,0,0,${ tools.random(0.05, 0.3) })`,
      ...
      setTimeout(generateItem, timeToNewItem * 1000)
    }
    generateItem()
  }
  ...
  drawNumbersAnimation() {
    ...
    state.animNumbers.forEach((item, i) => {
      ...
      tools.drawPath(ctx, () => {
        ...
        ctx.font = '18px Inter, Montserrat, sans-serif'
  ...
  drawAnimLines() {
    ...
    state.animLines.forEach((line, i) => {
      ...
      const color = `rgba(0,0,0,${ 0.05 + line.color * (p <= 0.5 ? p1 : p2) })`
  ...
  startGeneratingGlitches() {
    ...
    const generateItem = () => {
      ...
      const mp = state.mousePower
      const timeToNewItem = tools.random(
        (5 + 100 * (1-mp)) / Math.max(state.area, 0.1),
        (25 + 1200 * (1-mp)) / Math.max(state.area, 0.1)
      )
      
      const item = {
        p: 0,
        color: `rgba(0,0,0,${ tools.random(0.02, 0.15) })`,
  ...
  drawMouseMoveInteraction(props) {
    ...
    if (vector <= mouseRange) {
      const percent = (1 - (vector / mouseRange))
      ...
      const color = `rgba(0,0,0,${ 0.2 * percent })`
      const color2 = `rgba(0,0,0,${ 0.5 * p2 * percent })`
      ...
      tools.drawPath(ctx, () => {
        ctx.strokeStyle = `rgba(0,0,0,${ 0.3 * percent })`
  ...
  drawPlaneDotsAnimation(props) {
    ...
    const mf = 0.3 // Constant opacity for dots
  ...
  drawPlaneCenterLines() {
    const ctx = context.plane
    const { centerCoords } = plane
    
    tools.drawPath(ctx, () => {
      ctx.fillStyle = `rgba(0,0,0,0.05)`
      ctx.fillRect(centerCoords[0], 0, 1, doc.height)
      ctx.fillRect(0, centerCoords[1], doc.width, 1)
    })
  }
    ...
    for (let i = 0; i < cells[0]; i++) {
      for (let i2 = 0; i2 < cells[1]; i2++) {
        ...
        if (i !== yCenter && i2 !== xCenter) {
          this.drawPlaneDotsAnimation({i, i2, x, y})
        }
        if (i2 === xCenter && i !== yCenter) {
          this.drawYLines({i, i2, x, y})
          this.drawYMarkup({i, x, y})
        }
        if (i2 !== xCenter && i === yCenter) {
          this.drawXLines({i, i2, x, y})
          this.drawXMarkup({i2, x, y})
        }
      }
    }
  }
  ...
  drawMarkupYAnimation(props) {
    ...
    tools.drawPath(ctx, () => {
      ctx.fillStyle = `rgba(0,0,0,${ f * 0.2 })`
      ctx.fillRect(x, y - cfg.sectionWidth / 2, cfg.sectionHeight, cfg.sectionWidth)
    })
  ...
  drawMarkupXAnimation(props) {
    ...
    tools.drawPath(ctx, () => {
      ...
      ctx.fillStyle = `rgba(0,0,0,${ f * 0.15 })`
      ctx.textBaseline = 'middle'
      const textCoords = [x + (cfg.sectionWidth / 2) + cfg.numberOffset, y + cfg.sectionHeight / 2]
      ctx.fillText((xCenter - i2) + '', textCoords[0], textCoords[1])
    })
  ...
  draw() {
    ...
    // Draw base grid on plane canvas
    this.drawPlane()

    // Draw interactive elements on main canvas
    for (let i = 0; i < cells[0]; i++) {
      for (let i2 = 0; i2 < cells[1]; i2++) {
        const x = i * xCell
        const y = i2 * yCell
        ...
        if (i2 === xCenter && i !== yCenter) {
          this.drawMarkupYAnimation({i, i2, x, y})
        }
        if (i2 !== xCenter && i === yCenter) {
          this.drawMarkupXAnimation({i, i2, x, y})
        }
      }
    }
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    window.signalCanvas = new SignalCanvas()
  })
} else {
  window.signalCanvas = new SignalCanvas()
}